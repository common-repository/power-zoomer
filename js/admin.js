...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ pwz_upload_dir+"/"+file.name + '&task=pwz_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
	params += '&image_price='+jQuery('#multi_image_price').val() + '&image_dprice='+jQuery('#multi_image_dprice').val() + '&cpage='+
...
function on_queue_completed(file)
{
	...
	pwz_refresh_images_table(pwz_current_album_id);
...
function pwz_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=pwz_get_albums_table', function(res)
	{
		pwz_set_data_tables();
		if (res.status == 'ok') 
		{
...
function pwz_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=pwz_get_albums_images_table&album_id='+album_id, function(res)
	{
		pwz_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'pwz_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'pwz_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'pwz_reorder_image' )
		{
			pwz_refresh_images_table(album_id);
		}
		if( task == 'pwz_reorder_album' )
			pwz_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function pwz_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', pwz_submit_bulk);
...
function pwz_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'pwz_bulk_delete_albums' || task == 'pwz_bulk_enable_albums' || task == 'pwz_bulk_disable_albums')
	{
	...
	else if( task == 'pwz_bulk_delete_images' || task == 'pwz_bulk_enable_images' || task == 'pwz_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = pwz_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: pwz_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: pwz_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: pwz_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: pwz_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				...
				upload_url: pwz_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":pwz_upload_dir, "secw":pwz_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: pwz_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.pwz_delete_album').live('click', pwz_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', pwz_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', pwz_disable);
