...
if( !($album = $gpwz->pwz_get_album($album_id)) )
{
	die('Album id does not exists');
}
if( !($image = $gpwz->get_image($image_id)) )
{
...
<?php endif;?>
	...
	do_action("pwz_admin_head-$hook_suffix");
	do_action('pwz_admin_head'); 
	?>
</head>
<body>
<div id="wpwrap">
		...
		<div id="wpbody"><!-- <div id="wpbody-content">  -->
			<div class="wrap" style="padding: 12px;">
				...
				<form id="edit_album_form" action="<?php print admin_url() ?>" method="post" enctype="multipart/form-data">
					...
					<tr>
						<td><label><?php _e('Discount Price'); ?></label></td>
						<td><input type="text" id="image_dprice" name="image_dprice" value="<?php print @$image['dprice']; ?>" /></td>
					</tr>
					<tr>
						<td><label><?php _e('Image file'); ?></label></td>
						<td>
							<input type="file" name="image_file" value="" />
							<img src="<?php print pwz_get_album_url($album_id) . '/thumb/' . $image['thumb']; ?>" />
						</td>
					</tr>
					</table>
					<p>
