...
?>
<?php if(isset($gpwz->messages['upload']) && is_array($gpwz->messages['upload']) && count($gpwz->messages['upload']) > 0): ?>
<div class="updated">
<p class="">
<?php foreach($gpwz->messages['upload'] as $msg): ?>
	<span><?php print $msg; ?></span><br/>
<?php endforeach;?>
</p>
</div>
<?php endif; ?>
<script type="text/javascript">var pwz_current_album_id = <?php print $album['album_id']; ?>;</script>
<div class="dataTable_wrapper">
	<div class="fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix">
		...
		<?php pwz_get_table_actions(array('pwz_bulk_delete_images' => 'Delete', 
											'pwz_bulk_disable_images' => 'Disable', 
											'pwz_bulk_enable_images' => 'Enable')); ?>
	</div>
	...
	<tr>
		...
		<th><?php _e('Price'); ?></th>
		<th><?php _e('Discount Price'); ?></th>
		<th><?php _e('Order'); ?></th>
		<th><?php _e('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php require_once PWZ_PLUGIN_DIR . '/html/images_rows.php'; ?>
	</tbody>
	</table>
...
<div id="single_upload" class="tab_content">
	<form id="single_upload_form" action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="task" value="pwz_single_image_upload" />
		...
		<tr>
			<td><?php _e('Discount Price'); ?></td>
			<td><input type="text" name="image_dprice" value="" /></td>
		</tr>
		<tr>
...
<!--
	<form id="multi_upload_form" action="" method="post">
		...
		<fieldset>
			...
			<tr>
				<td><?php _e('Discount Price'); ?></td>
				<td><input type="text" id="multi_image_dprice" name="image_dprice" value="" /></td>
			</tr>
			</table>
			<fieldset>
...
-->
	...
	var pwz_upload_dir = "<?php  /* add the replace "\\" to work on windows server */  print str_replace("\\","/",pwz_get_album_dir($album['album_id'])); ?>";
	var pwz_secw = "<?php 
	global $wpdb;
	$query = "SELECT txt FROM {$wpdb->prefix}pwz_misc WHERE ione=1 AND itwo=1 AND ithree=1 LIMIT 1";
